...
@genType
and entities = {
    @as("Market") market: entityStoreOperations<Entities.Market.t>,
    @as("MarketActivity") marketActivity: entityStoreOperations<Entities.MarketActivity.t>,
  }
  ...
  let entities = {
      market: {
        makeStoreOperatorEntity(
          ...
          ~getStore=db => db->InMemoryStore.getInMemTable(
            ~entityConfig=module(Entities.Market)->Entities.entityModToInternal,
          )->(
            Utils.magic: InMemoryTable.Entity.t<Internal.entity> => InMemoryTable.Entity.t<
              Entities.Market.t,
            >
      ...
      marketActivity: {
        makeStoreOperatorEntity(
          ...
          ~makeMockDb=makeWithInMemoryStore,
          ~getStore=db => db->InMemoryStore.getInMemTable(
            ~entityConfig=module(Entities.MarketActivity)->Entities.entityModToInternal,
          )->(
            Utils.magic: InMemoryTable.Entity.t<Internal.entity> => InMemoryTable.Entity.t<
              Entities.MarketActivity.t,
            >
          ),
          ~getKey=({id}) => id,
        )
